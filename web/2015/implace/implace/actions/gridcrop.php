...
function implace_action_gridcrop_settings_cleanup ($settings) {
...
function implace_action_gridcrop ($settings) {
	...
	$grid_width = intval($settings[0][0]);
	$grid_height = intval($settings[0][1]);
	$grid_ratio = $grid_width/$grid_height;

	//remember: ratio > 1 = wide, ratio < 1 = tall
	if ($original_ratio >= $grid_ratio) {
		//wider than requested
		$desired_height = $grid_height;
		$desired_width = round(round($desired_height*$original_ratio)/$grid_width);
		$desired_width = $desired_width<1?1:$desired_width;
		$desired_width *= $grid_width;
	}else {
		//narrower than requested
		$desired_width = $grid_width;
		$desired_height = round(round($desired_width/$original_ratio)/$grid_height);
		$desired_height = $desired_height<1?1:$desired_height;
		$desired_height *= $grid_height;
	}
